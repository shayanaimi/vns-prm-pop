...
VnsPrmOPSolver<T>::VnsPrmOPSolver(CConfig& config, const std::string& problemFile) :
	...
	if (map_format == map_points_city_points) {
		...
		problem = OBSTOPLoader::getSOPDefinition(config, problemFile, true); //creates problem out of problemfile
		...
		problem = OBSTOPLoader::getSOPDefinitionMapFile(config, problemFile, true); //imports all the clusters
	...
	//at this point, each cluster is a single node 
	...
	for (int fromid = 0; fromid < num_clusters; ++fromid) {
		...
		for (int toid = 0; toid < nodesAllDistances[fromid].size(); ++toid) {
			if (planning_state == state2d || planning_state == state3d) {
				...
				nodesAllDistances[fromid][toid].resize(dubins_resolution * neighborhood_places);
				for (int head_id = 0; head_id < dubins_resolution * neighborhood_places; ++head_id) {
					nodesAllDistances[fromid][toid][head_id].resize(dubins_resolution * neighborhood_places); 
				}
			}
...
void VnsPrmOPSolver<T>::update_roadmap_distances() {
	...
	for (int fromcl = 0; fromcl < num_clusters; ++fromcl) { //for each cluster
		//INFO_VAR(fromcl);
		std::map<int, std::pair<int, int>> node_ids_poisitions;

		for (int from_node = 0; from_node < nodesAllClusters[fromcl].size(); ++from_node) { //for each node in the cluster
			//INFO_VAR(from_node); 
			int fromid = nodesAllClusters[fromcl][from_node].id;
			//cout << "size of cluster: " << nodesAllClusters[fromcl].size() << endl;
			...
			//std::cout << "found paths size" << found_paths.size() << std::endl;
